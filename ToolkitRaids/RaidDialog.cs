...
internal sealed class RaidDialog : Window
{
    ...
    public override Vector2 InitialSize => new(300, 300);
    ...
    {
        List<Raid> raids = _component!.GetActiveRaids().ToList();
        ...
        var viewPort = new Rect(0f, 0f, inRect.width - 16f, raids.Count * (Text.SmallFontHeight * 3f));
        ...
        for (var index = 0; index < raids.Count; index++)
        {
            ...
            float progress = raid.Timer / RaidMod.Instance.Settings.Duration;
    ...
    {
        if (Time.time % 2 < 1 && !_component!.GetActiveRaids().Any())
        {
            Close(false);
        }
    }
    ...
    {
        windowRect = new Rect(UI.screenWidth - InitialSize.x, UI.screenHeight / 2f - InitialSize.y / 2f, InitialSize.x, InitialSize.y);
    }
}
